...
# ── Telegram алерты ───────────────────────────────────────────────────────────
ALERT_BOT_TOKEN = "8838063815:AAFERm6rdfjKD5OWhr7ZNe11K6ptQHlYTvw"
ALERT_USER_ID   = 7995292747
...
async def handle_incoming(client: TelegramClient, account_id: int, me, event):
    ...
    try:
        ...
        try:
            c = sqlite3.connect(DB_PATH, timeout=15)
            c.execute("UPDATE contacts SET status='replied' WHERE id=?", (contact["id"],))
            c.commit()
            c.close()
        except Exception as e:
        ...
        try:
            ...
            draft = await asyncio.to_thread(generate_draft, our_msg, text, manager_name or "")
        except Exception as e:
    ...
    try:
        ...
        resp = await _ai.messages.create(
            ...
            messages=[{
                "role": "user",
                "content": (
                    "Слегка перефразируй это Telegram-сообщение от менеджера по ремонту квартир. "
                    "Правила:\n"
    ...
    while running:
        try:
            ...
            await loop.run_in_executor(None, lambda: subprocess.run(
                ["/opt/kitaiservise/venv/bin/python3", "sync_to_supabase.py"],
                cwd="/opt/kitaiservise/outreach",
