...
DB_PATH = Path(os.environ.get("OUTREACH_DIR", Path(__file__).parent)) / "outreach.db"
...
def init_db():
    ...
    for _stmt in (
        ...
        "ALTER TABLE accounts ADD COLUMN has_avatar INTEGER DEFAULT 0",
    ):
        try:
            conn.execute(_stmt)
            conn.commit()
...
def mark_contact(contact_id: int, status: str, account_id: int | None = None):
    ...
    with get_conn() as conn:
        if status == "sent":
            ...
            conn.execute("""
                UPDATE contacts SET status=?, replied_at=?
                WHERE id=?
