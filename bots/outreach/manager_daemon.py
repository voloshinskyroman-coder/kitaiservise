...
_patch_telethon_sqlite_timeout()
...
def _generate_variation_sync(template: str) -> str:
    ...
    payload = json.dumps({
        ...
        "messages": [{
            "role": "user",
            "content": (
                "Сделай МИНИМАЛЬНУЮ, лёгкую правку этого Telegram-сообщения первого контакта от "
                "менеджера компании Китай Сервис (доставка из Китая, растаможка). Единственная цель — "
                "чтобы текст не был дословно идентичен у разных менеджеров. Это НЕ редактура и НЕ "
                "переписывание — почти весь текст должен остаться таким же, только замени 2-4 слова "
                "на синонимы или чуть поменяй порядок слов в одном-двух местах.\n"
                "Правила (соблюдай строго):\n"
                "- Все слова в {фигурных_скобках} оставь без изменений, включая сами скобки\n"
                "- Сохрани структуру, все абзацы, пустые строки между ними и порядок предложений — 1 в 1\n"
                "- Не переписывай предложения целиком, не добавляй и не убирай смысловые куски\n"
                "- Не добавляй никаких новых заявлений, обещаний, слово 'бесплатно' — только синонимы к уже сказанному\n"
                "- Тон и стиль должны остаться ровно такими же, как в оригинале\n"
                "- Запрещены длинные тире (—), дефисы как разделители мыслей, канцелярит "
                "('данный момент', 'реализуем', 'оперативно', 'визит', 'пространство')\n"
                "- Если не знаешь, что поменять — оставь предложение как есть, лучше меньше правок, чем больше\n"
                "Отвечай только итоговым текстом, без пояснений и без кавычек вокруг него.\n\n"
                f"{template}"
            )
        }]
    }).encode()
